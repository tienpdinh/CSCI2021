...
int main(int argc, char *argv[]){
  ...
  int max_stride = stride1 > stride2 ? stride1 : stride2;
  int length = max_stride * iters;
  ...
  stride1_cpu_time  /= TRIALS;
  stride1_wall_time /= TRIALS;
  printf("%12d adds with stride1=%4d, CPU time: %.3f sec, Wall time: %.3f \n",
         iters,stride1,stride1_cpu_time,stride1_wall_time);
  ...
  stride2_cpu_time  /= TRIALS;
  stride2_wall_time /= TRIALS;

  printf("%12d adds with stride2=%4d, CPU time: %.3f sec, Wall time: %.3f \n",
         iters,stride2,stride2_cpu_time,stride2_wall_time);
